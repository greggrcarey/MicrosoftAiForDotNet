...
var model = config["EmbeddingModelName"];

IEmbeddingGenerator<string, Embedding<float>> generator = new OpenAIClient(key).GetEmbeddingClient(model).AsIEmbeddingGenerator();
...
await cloudServicesStore.CreateCollectionAsync();
